\def\ProofInAppendix{TRUE} \def\CGT{1}%
\input{prefix}%
...
\CGTVer{%
   \authorrunning{S. Ashur S. Har-Peled}
...
graph $\G=(\PS, \EG)$, such that for any region $\region\in\FF$ (i.e., the
...
graph $\G = (\PS,\EG)$ is an \emphw{$\FF$-local $t$-spanner} for $\PS$,
if for any $\region \in \FF$, the subgraph of $\G$ induced on
...
In the final result, described in \secref{rectangles},
we use a lesser known type of pair-decomposition to construct a weak local
spanner for axis parallel rectangles.
Here, the graph $\G$, constructed over $\PS$, has the
property that for any axis-parallel rectangle $\rect$, the graph
$\restrictY{\G}{\rect}$ is a $(1+\eps)$-spanner for all the points of
...
where this form of multiplicative shrinking is still meaningful (unlike
the diameter based shrinking mentioned above). Contrast this with the
lower bound (illustrated in \figref{bad:rectangles}) of $\Omega(n^2)$
on the size of local spanner if one does not shrink the rectangles.
See \secref{rectangles} for details of the precise result.
...
\NotCGTVer{The following claim is straightforward.}
\begin{lemma}
    \lemlab{chop:easy}%
    %
    ...
    $|\WS'| = \Of(\beta^d|\WS|)$ and
    $\WeightX{\WS'} = \Of(\beta^d\WeightX{\WS})$.
\end{lemma}
\CGTVer{%
   \begin{proof}
...
\begin{proof}
		\begin{figure}[ht]
			...
			\caption{An illustration of refining the pairs in a \SSPD into
				...
				$\eps$-double-wedge. $\PSX$ is contained in the green disk,
				while $\PSY$ is contained in the red square, and the
		...
		By using \lemref{chop:easy}, we can assume that $\WS$ is
		$(\constA/\eps)$-semi separated for some constant $\constA$. For each pair
		$\Pair = \{ \PSX, \PSY \} \in \WS$, assume that
		$\diameterX{\PSX} < \diameterX{\PSY}$. Now, the algorithm scans
		the pairs of $\WS$ and performs the following procedure for each
		one. Let $\disk$ be the smallest axis-parallel disk containing
		...
		$\eps/2$, see \figref{partition}. This partitions the plane
		into a set of cones $\ConeSet$. For a cone $\cone \in \ConeSet$,
		The algorithm now adds the pair $\{ \PSX, \PSY \cap \cone \}$. 
		The constructed set is clearly $(\constA/\eps)$-semi separated,
		and is of the right size and weight, and construction is straight
		forward assuming $\WS$ is already constructed.
		
		We now show that there exists an $\eps$-double-wedge that contains
		$\PSX$ in one side, and $\PSY \cap \cone$ in the other.
		To see that we first assume without loss of generality that $\PSX$
		is contained in a disk of diameter 1. Now, let $\arc$ be the arc
		created by $\cone$ on the circle with radius $\constA/\eps$ around
		$\origin$, let $\seg$ be the segment supporting $\arc$, and let
		$\pa$ be the center of the segment between $\origin$ and the
		center of $\seg$. Note that this segment bisects $\cone$. By
		choosing $\constA$ large enough, $\seg$ is of length $\geq 1$.
		Consider the double wedge created by the two lines defined each by
		an endpoint of $\seg$ and $\pa$. The angle of that double wedge is
		at most double that of $\cone$, since the circle centered at $\pa$
		and passing through the endpoints of $\seg$ has $\origin$ and
		$\pa$ on its diameter, both defining an arc of the same length,
		and since $\origin$ is in the interior of this circle while $\pa$
		is its center, we get the required bound. Now, notice that by the
		choice of $\seg$ we have that $\PSX$ and $\PSY \cap \cone$ are
		indeed contained in opposite wedges of the double wedge, as required.     
		
		\begin{figure}[h]
			\centering
			\includegraphics[width=0.48\linewidth]{../figs/double_wedge_cgt}%
			\caption{An illustration of the proof for \lemref{refine:d:w}. The dashed arc is a part of the disk not containing points of $\PSY$ due to the semi separated property. $\PSX$ is contained in the green disk, $\cone$ is shown in blue, and the $\eps$-double wedge is shown in red.}
			\figlab{double-wedge}
		\end{figure}
...
\begin{lemma}
	...
	$\PS$, in $\Of( (\eps^{-1} + \delta^{-2}) n \log n)$ time, and with
	$\Of\bigl( (\eps^{-1} + \delta^{-2}) n \bigr) $ edges, such that
	for any (bounded) convex region $\body$ in the plane, we have that for any
	two points $\pa, \pb \in \PS \cap \shrinkDY{\body}{\delta}$ the
	graph $\restrictY{ \body }{\PS}$ has a $(1+\eps)$-path between
	$\pa$ and $\pb$. 
\end{lemma}
\begin{proof}
...
\begin{lemma}
   ...
   Let $\CC$ be a convex bounded body, and let $\PS$ be a set of
	points in general position with respect to $\CC$. Then, if $C$ is a
	homothet of $\CC$ that contains two points
...
\begin{claim}
   ...
   Let $\CC$ be a compact (bounded and closed) convex body.  Given a
	...
	triangulation of $\PS$. For any homothet $C$ of $\CC$, we have that
...
$\eps \in (0,1)$, and a convex body $\CC$ that defines the ``unit''
ball. The task is to construct $\CC$-local spanner.
...
$\DT_{\Pair} = \DG_{\CC}(\PSX \cup \PSY)$, and adds all the edges in
$\DT_{\Pair} \cap (\PSX \otimes \PSY)$ to the computed graph $\G$.
...
Our algorithm constructs $\G$, the graph over $\PS$ formed by connecting every point
...
\begin{proof}
	...
	smaller than $\pi/3$ (for an appropriate choice of $\gamma$),
	we have that
	...
	\angleX{\pa}$. Similarly,
	$\angleX{\pb} \in [\alpha, \pi -\alpha]$. Since $\sin$ is $1$-Lipshitz,
	and as $\sin x \approx x$, for small $x$, and for $\gamma$
...
\begin{proof}
	Consider the closest pair $\pa,\pb \in\PS' =  \PS \cap \triangle'$. They
	must be connected directly in $\G'$, as otherwise there is a point
	$\pc \in \PS'$ in the cone containing the
...
%$\pa \in \PS$ to $\nnZ{i}{\pa}{\cone}$, for all $i \in \IRX{3}$ and
%$\cone \in \ConeSet_i$.  We get the following result.
...
\begin{theorem}
	...
	$\eps \in (0,1)$ be an approximation parameter. The algorithm 
	described in this section computes a$\triangle$-local
	$(1+\eps)$-spanner $\G$ for	an $\alpha$-fat triangle $\triangle$.
	The construction time is $\Of\pth{(\alpha\eps)^{-1}n\log n}$,
	and the spanner $\G$ has $\Of\pth{(\alpha\eps)^{-1} n\bigr.}$ edges.
\end{theorem}
...
\begin{proof}
    ...
    $\triangle$-local $(1+\epsA)$-spanner for $\PS$, it follows readily
    ...
    $\Body''$. Then, by \lemref{narrow:traps:decomp} there is a
    $\epsA$-narrow trapezoid $\Trap'$
...
\end{lemma}
\begin{proof}
	The construction algorithm recurses on the dimensions, using
	...
	This set of pairs is indeed quadrant separated as all the pairs in
