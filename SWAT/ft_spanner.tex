\def\ProofInAppendix{TRUE} \def\SWAT{1}%
\input{prefix}%
...
\begin{abstract}
    For a set $\PS \subseteq \Re^2$ of points and a family $\FF$ of
    regions, a \emph{local $t$-spanner} of $\PS$ is a sparse graph $\G$
    ...
    set. We prove that this dependency cannot be removed, thus
    ...
    \stav{The authors use phrases "region", "body", "set" inconsistently. Please make the use of those phrases consistent. When we say "body", we usually mean that it is compact. So, saying "bounded convex body" sounds repetitive and weird.}
\end{abstract}
...
graph.  Here, an edge $\pa \pb \in \binom{\PS}{2}$ is associated with the segment
...
between $\pa$ and $\pb$ in $\GA$ is a \emphw{$t$-path} if the length
of $\pi$ in $\GA$ is at most $t\cdot\dGZ{\G}{\pa}{\pb}$, where
$\dGZ{\G}{\pa}{\pb}$ is the length of the shortest path between $\pa$
and $\pb$ in $\G$.  The graph $\GA$ is a \emphw{$t$-spanner} of $\G$
if there is a $t$-path in $\GA$ for every $\pa,\pb\in \PS$.  Thus,
for a set $\PS\subseteq \Re^d$ of points, a graph $\G$ over $\PS$ is a
...
(See \defref{def:residual:graph} for a formal definition of this
...
faults, meaning that some arbitrary set of at most $k$ of
vertices and edges has failed. Levcopoulos \etal \cite{lns-iacfts-02}
showed the existence of $k$-vertex/edge fault tolerant spanners for a
set $\PS$ of points in some metric space. Their spanner had
$\Of(kn\log n)$ edges, and weight, i.e. sum of edge weights, bounded
by $f(k)\cdot wt(MST(\PS))$, where $wt(MST(\PS))$ is the weight of
$MST(\PS)$, for some function $f$. Lukovszki
...
property to being fault-tolerant.  For a family $\FF$ of regions, a
graph $\G = (\PS,\EG)$ is an \emphw{$\FF$-local $t$-spanner} for
...
In the final result we use a less known type of pair-decomposition to
...
with the lower bound (illustrated in \figref{bad:rectangles}) of
$\Omega(n^2)$ on the size of local spanner if one does not shrink the
rectangles.  See \secref{rectangles} for details of the precise
result.
...
\begin{definition}
    ...
    the plane. For a region $\region \in \FF$ and a geometric
    graph $\G$ on a point set $\PS$, let $\G \gminus \region$ be the
    residual graph after removing from $\G$ all the points of $\PS$ in
...
The \emphi{closest pair} distance of a set
$\PS \subseteq \Re^d$ of points is
\begin{math}
...
\begin{defn}
    ...
    $\eps$ is a pair decomposition of $\PS$ with a set
    \begin{math}
        \WS = \brc{\bigl.
           \brc{\PSB_1,\PSC_1},\ldots,\brc{\PSB_s,\PSC_s}},
    \end{math}
    of pairs such that for all $i$, the sets $\PSB_i$ and $\PSC_i$ are
    $(1/\eps)$-well-separated. The notion of $(1/\eps)$-\SSPD (a.k.a.
    \emphi{semi-separated pair decomposition}) is defined
    analogously.
\end{defn}
...
\SWATVer{The following claim is straightforward.}
\begin{lemma}
    \lemlab{chop:easy}%
    %
...
\NotSWATVer{%
   \begin{proof}
       ...
       $\diameterX{\PSX} < \diameterX{\PSY}$. Let $\sqr$ be the
       smallest axis-parallel cube containing $\PSX$, and denote its
       sidelength by $r$.  Let $r' = r / \ceil{\sqrt{d} \beta }$.
       Partition $\sqr$ into a grid of cubes of sidelength $r'$, and
       let $T_\Pair$ be the resulting set of squares. The algorithm
       now add the set of pairs
       \begin{equation*}
...
\begin{theorem}
    \lemlab{refine:d:w}%
	%
    \InFullVer{}%\SWATVer{(Proof in full version \cite{ah-lsr-22}.)}
    Given a
    $(1/\eps)$-\SSPD $\WS$ of $n$ points in the plane, one can refine
    $\WS$ into a $(1/\eps)$-\SSPD $\WS'$, such that each pair
    $\Pair = \{ \PSX, \PSY \} \in \WS'$ is contained in an
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    (e.g., a disk).  A \emphi{homothet} of $\CC$ is a scaled and
    translated copy of $\CC$.  A point set $\PS$ is in \emphi{general
       position} with respect to $\CC$, if no four points of $\PS$ lie
    on the boundary of a homothet of $\CC$, and no three points are
    colinear.


    A graph $\G=(\PS, \EG)$ is a \emphw{$\CC$-local $t$-spanner} for
...
\begin{theorem}[\cite{cd-vdbcdf-85}]
    ...
    %
    For a set $\PS$ of points, $\DG_{\CC}(\PS)$ can be computed in
    $\Of(n \log n)$ time for a pre-determined convex body $\CC$.
...
\begin{lemma}
    ...
    $\pa, \pb \in \PS$, then there exists a homothet
    $C' \subseteq C$ of $\CC$ such that $\pa, \pb \in \partial C'$.
\end{lemma}

See \figref{shrink:h} for An illustration of the claim in \lemref{shrink:shrank}.
...
\begin{claim}
    ...
    set $\PS \subseteq \Re^2$ of points in general position with
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    the algorithm of \lemref{s:s:p:d:spread}, where $\epsA = \eps/6$.
...
\begin{proof}
    ...
    $\px'\py' \in \EGX{ \DT_\Pair}$. Indeed, let $\px \in \PSX \cap C$, $\py \in \PSY \cap C$, and let $\pi$ be some $(\px,\py)$-path guaranteed to exist by connectivity. $\pi$ must contain an edge with one endpoint in $\PSX \cap C$ and the other in $\PSY \cap C$. As such, by construction, we have
...
\begin{theorem}
    ...
    given set of $n$ points in the plane (in general position with respect to $\CC$), and
...
\NotSWATVer{%
   ...
   The following defines a ``visibility'' graph when we are restricted
   to a region $R$, where two points are visible if there is a witness
...
\NotSWATVer{%
   \begin{proof}
       ...
       $\restrictY{\G}{C_i} \subseteq
       \restrictY{\G}{R}$. Concatenating these paths directly yields
       the desired result.
...
of Abam \etal \cite{abfg-rftgs-09} and use an \SSPD instead of a \WSPD,
...
{%
   ...
   set $\PS$ of $n + \ceil{\log_2 \spread}$ points in the plane, with
...
% \RestatementOf{\lemref{l:s:lower:bound}}% {%% \LemmaDiskLowerBound{} }
...
%
    ...
    $x_j = n2^{M-j+1} = x(\pb_{j-1})/2$\stav{$x(q_{j-1})$--> $x_{j-1}$}, and let $y_j < 0$ be the
    ...
    (i.e., $\dY{\pa_1}{\pa_2}\bigr.$). Indeed $x(\pb_{M-1}) = 4n$\stav{$x(q_{M-1})$--> $x_{M-1}$} and
    ...
    observe that for any index $k \in \IRX{j-1} = \brc{1,\ldots,j-1}$, we have that
    \begin{equation*}
        \dY{\pa_i}{\pb_k} %
        =%
    ...
    between $\pa_i$ and $\pb_j$, cannot use any of the points
...
\NotSWATVer{%
   See \figref{tri_low_bd} for an illustration of the construction
   described in \lemref{l:b:triangles} showing the dependency between
   the spread and the number of edges if "thin" triangles are allowed.
}
...
does not depend on the spread of the points.
...
where $\beta = \eps \alpha/\gamma$, and $\gamma>1$ is some constant
discussed shortly. For each point $\pa\in\PS$, and a cone
...
\SWATVer{full version \cite{ah-lsr-22} for details}%
\NotSWATVer{\apndref{l:s:triangle:details} for details}).
...
\begin{lemma}
    ...
    \defref{angular:separated}), and let $\Trap$ be a $\epsA$-narrow
    ...
    \begin{compactenumI}
        ...
        triangulation of $\PX \cup \PY$, and
...
\NotSWATVer{%
   \begin{proof}
       We show a somewhat suboptimal but simple construction. A
       $t$-nice polygon has at most $t$ edges.  Let $\psi$ be the
       sensitivity of $\Body$, and place a minimum set $\PS$ of points
       on the boundary of $\Body$, which includes all the vertices of
       $\Body$, and such that the distance between any consecutive
       pair of points is in the range $[\constA, 2\constA]$, where
       $\constA = \epsA \psi/ \constB$, for some sufficiently large
       constant $\constB$. In particular, let
       $M =\max_{e \in \EGX{\Body}} \ceil{\| e\| / \constA} = \Of(
       1/\epsA)$.

       In addition, place $\constC \cdot t$ equally spaced points
       between any two consecutive points of $\PS$, where $\constC$ is
       a constant to be determined shortly. Let $\PSA$ be the set
       resulting from $\PS$ after adding all these points.
       ...
       \begin{figure}[h]
           ...
           \hfill%
           \phantom{}%
           \caption{The points of $\PS$ (round), and all the points
              added to $\PS$ in order to create $\PSA$ (square). On
              the right, a ``vertical'' decomposition induced by one
              of the directions of $\PS \times \PSA$.}
           \figlab{v:decompose}
       \end{figure}

       We have that $|\PS| = \Of( t /\epsA)$ and
       $|\PSA| = \Of(t^2/\epsA)$. For a direction $v$, let $\Traps_v$
       be the decomposition into trapezoids formed by shooting rays
       from inside $\Body$ in the direction of $v$ (or $-v$) from all
       the points of $\PS$, see \figref{v:decompose}. Let $\Traps_v'$
       be the set resulting from throwing away trapezoids with legs
       that lie on adjacent edges.  It is easy to verify that all the
       trapezoids of $\Traps_v'$ are $\epsA$-narrow.  Let $U$ be the
       set of all directions induced by pairs of points of
       $\PS \times \PSA$, and let $\Traps = \cup_{u \in U}
       \Traps_u'$. We have that
       ...
       $\PS$ such that $\pa \in \pa_1 \pa_2$.  Now, let $\pb_1, \pb_2$
       be the adjacent points of $\PSA$ such that
       $\pb \in \pb_1 \pb_2$.  We assume that
       $\pa_1, \pa_2, \pb_1,\pb_2$ are in this clockwise order along
       ...
       triangle with sidelength $\psi$ and base angle $\pi/t$. As
       such, the height of such a triangle is
       ...
       \begin{figure}[h]
           ...
           \caption{The height of the triangle
              $\triangle \pa_1 \pa_2 \pb_2$ is minimized as $\pb_2$
              and $\pa_1$ are moved to vertices of $\Body$.}%
           \figlab{points:move}
       \end{figure}
       ...
       \figref{points:move}. Assume, for concreteness, that $\pa_1$ is
       a vertex of $\Body$, and observe that
       ...
       least $h' = h/M = \Omega( \eps \psi/t )$. The quantity $h'$ is
       a lower bound on the length of the projection of $\pa_1 \pa_2$
       on the line spanned by $\pb_1 \pb_2$. However,
       $\dY{\pb_1}{\pb_2} \leq 2\constA/\constC t = \Of( \epsA \psi
       /\constC t) < h'$, by picking $\constC$ to be a sufficiently
       large constant.

       This readily implies that the trapezoid induced by the
       direction $ u = \overrightarrow{\pa_1 \pb_2}$ in $\Traps_u'$
       that contains $\pa$ on one of its leg, and $\pb$ on the other.
   \end{proof}%
}
...
\begin{proof}
    ...
    induced by a vertex of $\Body$, and a non-adjacent edge of
    ...
    We compute a $\epsA$-angular $(1/\epsA)$-\SSPD $\WS$
    ...
    a desired spanner.  The construction time is
    \begin{equation*}
    ...
    $\Body''$. Then, there is a $\epsA$-narrow trapezoid $\Trap'$
    ...
    \begin{equation*}
        ...
        (1+\eps) \dY{\pa}{\pb},
    \end{equation*}
    which implies that there is $(1+\eps)$-path from $\pa$ to
    $\pb$ inside $\Body'$.
    % \end{itemize}
\end{proof}%
}
...
\seclab{rectangles}
% \apndlab{rectangles}
\subsection{Orthant separated pair decomposition}
\seclab{qspd}
% \apndlab{qspd}
...
$\pb$ \emphi{dominates} $\pa$ coordinate-wise. That is $p_i < q_i$,
for all $i$. More generally, let $\pa <_i \pb$ denote that
$\pa_i < \pb_i$. For two point sets $\PSX, \PSY \subseteq \Re^d$, we
use $\PSX <_i \PSY$ to denote that $\px <_i \py \quad \forall \px \in \PSX,
\py \in \PSY$. In particular
$\PSX$ and $\PSY$ are \emphw{$i$-coordinate separated} if
$\PSX <_i \PSY$ or $\PSY <_i \PSX$. A pair $\{ \PSX, \PSY\}$ is
\emphi{orthant-separated}, if $\PSX$ and $\PSY$ are $i$-coordinate
separated, for all $i=1,\ldots, d$.

A \emphi{orthant-separated pair decomposition} of a point set
...
orthant-separated for all $i$.

\SWATVer{In the full version of the paper \cite{ah-lsr-22}, }%
\NotSWATVer{In \apndref{l:s:qspd:details}, }%
we prove the properties regarding the computational and combinatorial
complexity of \QSPD{}s that are used in the proof of
\thmref{a:l:s:rectangles}%}.
...
\subsubsection{Construction for a single orthant separated pair}
\seclab{pair:edges}
...
\begin{figure}[t]
	...
	\caption{The construction of the grid $\grid(\pa,\Pair)$ for a
		...
		%in the construction.
         ...
         $\PS$. The set $\PSX$ is orthant-separated from $\PSY$, that
         is, there is a point $\cen_\Pair$ such that $\PSX$ and $\PSY$
         are contained in two opposing orthants in the partition of
         ...
         orthant, and the points of $\PSY$ are in the positive
         orthant.


         For a point $\pa = (-x,-y)\in \PSX$ we construct a
         ...
         \subsubsection{The spanner construction algorithm}
...
$\xSlabX{\rect} = \Set{(x,y)\in \Re^2}{\exists x'\in\Re \text{ such that } (x',y) \in \rect}$ be
...
\begin{equation*}
    \ySlabX{\rect} = \Set{(x,y)\in \Re^2}{\exists y'\in\Re \text{ such that } (x,y') \in \rect}.
\end{equation*}
...
\begin{lemma}
    ...
    % 
    \InFullVer{}%
    Assume that $\delta < 1/2$, and
    $\gConst \geq \ceil{20/\eps + 20/\delta}$.  Consider a pair
    $\Pair = \{\PSX, \PSY \}$ in the above construction, and a point
    $\pa =(-x,-y) \in \PSX$ with its associated grid
    $\grid = \grid(\pa, \Pair)$. Consider any axis-parallel rectangle
    $\rect$, such that $\pa \in (1-\delta)\rect = I\times J$, and
    $(1-\delta)\rect$ intersects a cell $\cell \in \grid$. We have the
    following.
    \begin{enumerate}
        \smallskip%
...
\begin{proof}
    ...
    follow by symmetry from \itemref{x:bigger} and \itemref{n:w},
    respectively.
...
\end{lemma}
\NotSWATVer{%
\begin{proof}%{\lemref{local:spanner}}{local:spanner:x}
...
\end{theorem}
\NotSWATVer{%
   \begin{proof}%{\thmref{a:l:s:rectangles}}{rectangles:3}
       Computing the \QSPD $\WS$ takes $\Of(n \log^2 n)$ time. For
       each pair $\{\PSX, \PSY\}$ in the decomposition with
       $m = |\PSX| + |\PSY|$ points, we need to compute the lowest and
       leftmost points in $(\PSX \cup \PSY) \cap \cell$, for each cell
       in the constructed grid. This can readily be done using
       orthogonal range trees in $\Of( \log^2 n)$ time per query (a
       somewhat faster query time should be possible by using the
       offline nature of the queries, etc). This yields the
       construction time. The size of the computed graph is
       $\Of(\WeightX{\WS} \gConst^2) = O\bigl((1/\delta^2 + 1/\eps^2)
       n \log^2 n\bigr)$.
...
\apndlab{l:s:triangle:details}


\begin{lemma}%[Proof in \apndref{cone_edge:triangles}]
...
\begin{proof}
	...
	The other possibility, illustrated in
        \figref{tri_cone_edge:b}, is that
...
\begin{lemma}%[Proof in \apndref{shrink:triangles}]
...
\paragraph*{Size and running time}
 ...
 \RestatementOf{\thmref{l:s:triangle}}%
 {%%
	 \ThmLSTriangle{} }
...
\NotSWATVer{%
\section{Missing details about orthant separated pair decompositions}
\apndlab{l:s:qspd:details}
...
\begin{proof}
	...
	$\QSup, \QSdown$ are orthant separated by induction, and pairs in
	$\widehat{\QS}$ are orthant separated in the first $d-1$
	...
	corresponds to this pair orthant-separates $\pa$ and $\pb$ as
	desired.
