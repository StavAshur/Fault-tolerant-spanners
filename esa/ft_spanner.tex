\def\ProofInAppendix{TRUE} \def\ESA{1}%
\input{prefix}%
...
if there is a $t$-path in $\GB$, for every $\pa,\pb\in \PS$.  Thus,
...
graph $\G = (\PS,\EG)$ is a \emphw{$\FF$-local $t$-spanner} for $\PS$,
...
In the final result, moved to \apndref{w:l:s:rect} due to space constraints,
we use a lesser known type of pair-decomposition to construct a weak local
spanner for axis parallel rectangles.
Here, the constructed graph$\G$ over $\PS$, has the
...
See \apndref{w:l:s:rect} for details of the precise result.
...
\ESAVer{The following claim is straightforward.}
\begin{lemma}
    \lemlab{chop:easy}%
    %
...
\SaveContent{\LemmaWLSRegions}{%
   ...
   for any (bounded) convex $\body$ in the plane, we have that for any
   two points $\pa, \pb \in \PS \cap \shrinkDY{\body}{\delta}$ the
   graph $\restrictY{ \body }{\PS}$ has a $(1+\eps)$-path between
   $\pa$ and $\pb$.  }

\begin{lemma}[Proof in \apndref{w:l:s:regions}]
...
\begin{lemma}[Proof in \apndref{shrink:shrank}]
...
\begin{claim}[Proof in \apndref{c:t:connected}]
...
\begin{theorem}\RefProofInAppendix{p:k:gon}
    \thmlab{k:gon}%
    %
...
\end{theorem}
\begin{proof:in:appendix:e}{\thmref{k:gon}}{p:k:gon}
...
\subsubsection{Quadrant separated pair decomposition (QSPD)}
...
\begin{lemma}[\cite{aesw-emstbcp-90}]
	Given a set $\PS$ of $n$ points in $\Re^d$, one can compute, in
	$\Of( n \log^d n)$ time, a \QSPD of $\PS$ with
	$\Of(n \log^{d-1} n)$ pairs, and of total weight
	$\Of( n \log^d n)$.
...
\subsection{Proof of \TPDF{\lemref{refine:d:w}}{{refine:d:w}}}
\apndlab{refine:d:w}
...
\subsection{Proof of \TPDF{\lemref{w:l:s:regions}}{{w:l:s:regions}}}
\apndlab{w:l:s:regions}

\RestatementOf{\lemref{w:l:s:regions}}%
{%
   \LemmaWLSRegions{}%
}
...
\subsection{Proof of \TPDF{\lemref{shrink:shrank}}{{shrink:shrank}}}
\apndlab{shrink:shrank}

\RestatementOf{\lemref{shrink:shrank}}%
{%
   \LemmShrinkShrank{}%
}
...
\subsection{Proof of \TPDF{\clmref{c:t:connected}}{{c:t:connected}}}
\apndlab{c:t:connected}
