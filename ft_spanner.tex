...
%
%\begin{lemma}[4.1.4 \cite{ns-gsn-07}]
%	\lemlab{theta_graph}%
...
\begin{proof}
	Let $\triangle\subseteq\Re^2$ be a triangle $\pa,\pb\in \PS\cap \triangle$ be two points, and let $\triangle'$ be a copy of $\triangle$ containing, say, $\pa$ on one of its vertices, and $\pb$ on the edge opposite to that vertex (as in \lemref{shrink:triangles}). We prove using induction on the rank of $|\overline{\pa\pb}|$.
	
	If $\pa$ and $\pb$ are the closest pair, then $\overline{\pa\pb}$ must be an edge in the resulted graph. This is because $\pb$ is contained in one of the cones in $C_\pa$ by construction, and if there exists a point $\pc\in C_\pa$ that was connected to $\pa$, then by \lemref{theta_graph} we get that $|\overline{\pa\pb}|\geq |\overline{\pc\pb}|$.
	
	Otherwise, by \lemref{theta_graph} we have that $\pa$ is connected by an edge to a point $\pc\in \triangle$
\end{proof}
