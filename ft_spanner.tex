...
\end{figure}
\begin{proof}
    (I) Let $\DT = \DG_\CC(\PX \cup \PY)$.  \clmref{c:t:connected}
    implies that $\DT \cap \Trp$ is connected. Thus, there is a path
    in $\DT \cap \Trap$ between $\pa$ and $\pb$, and thus, there must
    be an edge $\pa'\pb'$ along this path with $\pa' \in X$ and
    $\pb' \in Y$.

    (II) Let $\ell = \dY{\pa'}{\pb'}$. Assume for concreteness that
    $\diamX{\PX} \leq \epsA \dsY{\PX}{\PY} \leq \epsA \ell$. Let $\pb''$ be
    the closest point on $\pa\pb$ to $\pb'$. We have that
    \begin{equation*}
        \dY{\pa}{\pb''}
        \geq
    ...
    BUG: We are NOT using the narrowness of the trapezoid... 
\end{proof}
