...
\begin{lemma}
    ...
    $O((t/\epsA)^2)$ $\epsA$-narrow trapezoids, such that for any two
...
\begin{proof}
    ...
    In addition, place $c t$ equally space points between any two
    consecutive points of $\PS$, where $c$ is constant to be
    determined shortly. Let $\PSA$ be the resulting set from $\PS$
    ...
    consecutive vertices. In the worst case, this is an equilateral
    triangle, with isosceles triangle with sidelength $\psi$ and side
    angle $\pi/t$. As such, the height of such a triangle is
    $h = \psi \sin( \pi/t) \geq \psi/t$. It follows that the height of
    the triangle $\triangle \pa_1 \pa_2 \pb_2$ is at least
    $h' = h/M = \Omega( \eps \psi/t )$. The quantity $h'$ is a lower
    bound on the length of the projection of $\pa_1 \pa_2$ on the line
    spanned by $\pb_1 \pb_2$. However,
    $\dY{\pb_1}{\pb_2} \leq 2\gamma/ct = O( \epsA \psi /ct) < h'1$, by
    picking $c$ to be sufficiently large constant.

    This readily implies that the trapezoid induced by the direction
    $\overrightarrow{\pa_1 \pb_2}$ contains both $\pa$ and $\pb$ on
    its legs, which implies the claim.
\end{proof}
