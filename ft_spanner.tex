...
\begin{proof}
	First, we describe two sets of points $A$ and $B$ of size $n$ each (with some abuse of notation), such that every point in $A$ must be connected to $O(n)$ points of $B$ in any $\triangle$-local $(1+\eps)$-spanner for a triangle that we will choose accordingly. 
	Let $A = \{a_i:=(-i, i/n) ~|~ 0\leq i \leq n \}$, and let $B = \{b_i:=(n-i, 1+2^i) ~|~ 1\leq i \leq n \}$.
	...
	Notice that for every point $a\in A$ and index $i\in \{0,..,n\}$, There exist a homothet $\triangle'$ of $\triangle$ such that $B\cap \triangle'=\{b_i,...,b_n\}$. 
