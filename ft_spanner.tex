...
\SaveContent{\LemmaRefineDWBody}{ Given a $\eps^{-1}$-\SSPD $\WS$ of
   $n$ points in the plane, one can refine it, into a
   $\eps^{-1}$-\SSPD $\WS'$, such that each pair
...
A trapezoid $\triangle$ is \emphi{$\epsA$-narrow} if the length of one
of its edges at most $\epsA \diamX{\Trap}$, which is the \emphi{short}
edge of $\triangle$, the vertex facing the short edge is the triangle
\emphi{head}.

\begin{lemma}
    \lemlab{shrink:triangle}%
    %
...
\end{lemma}
\begin{proof}
    Part (I) is a ready consequence of \clmref{c:t:connected}.  By
    \lemref{shrink:triangle}, this edge is realized by a homothet
...
$\eps \in (0,1)$. Let $v_i$ denote the $i$\th vertex of $\triangle$,
$\alpha_i$ be the adjacent angle, and let $e_i$ denote the facing
edges, for $i\in \IRX{3}$.  Let
...
$\triangle$ orthogonal to $e_i$.  We assume that the smallest angle in
$\triangle$ is at least $\alpha$.  See \figref{tri_cones} for an
illustration. Let $\ConeSet_i$ be a minimum partition of $\cone_i$
into cones each with angle in the range $[\beta, 2\beta]$, where
$\beta = \eps \alpha/\constB$ and $\constB$ is a constant to be determined
shortly.  For each point $\pa\in\PS$, and a cone
...
\begin{proof}
    ...
    $\theta_\pc$ respectively.  Since the angle of $\cone$ is smaller
    than $60$ degrees, we have that
    ...
    Consider the angles $\theta_\pa = \angle \pc \pa \pb$,
    $\theta_\pc = \angle \pa \pc \pb$, and
    $\theta_\pb = \angle \pa \pb \pc$.  Observe that
    $\theta_\pc \leq \theta_\pb$ as $\dY{\pa}{\pb}\leq \dY{\pa}{\pc}$.
    As such $\theta_\pc \leq \pi/2$.  Furthermore,
    $\theta_\pc \geq \alpha \gg \eps \alpha/\constB = \beta \geq
    \theta_\pa$. Similarly, $\theta_\pb \in [\alpha, \pi -\alpha]$.
    By the $1$-Lipshitz of $\sin$, and as $\sin x \approx x$,
    for small $x$, and for $\constB$ sufficiently large. we have that
    \begin{equation*}
        \sin( \theta_\pb + \theta_\pa) \in [1-\eps/4, 1+\eps/4] \sin
        \theta_\pb%
        \qquad\text{and}\qquad
        \sin \theta_\pa \leq (\eps/4) \sin \theta_\pc.
    \end{equation*}
    As such, by the
    law of sines, we have that
    \begin{math}
        \frac{\dY{\pb}{ \pc}}{\sin{\theta_\pa}}%
        =%
        \frac{\dY{\pa}{ \pb}}{\sin{\theta_\pc}}%
        =%
        \frac{\dY{\pa}{ \pc}}{\sin{\theta_\pb}}.%        
    \end{math}
    This implies that
    \begin{equation*}
        \dY{\pa}{\pc}%
        ...
        (1+\eps)\dY{\pb}{\pc}%
        =%
        \pth{\frac{\sin{\theta_\pb}}{\sin{\theta_\pc}} +(1+\eps)
           \frac{\sin{\theta_\pa}}{\sin{\theta_\pc}} }\dY{\pa}{ \pb}.        
    \end{equation*}
    Observe, by the above that
    \begin{equation*}
        \frac{\sin{\theta_\pb}}{\sin{\theta_\pc}}
        +(1+\eps)
        \frac{\sin{\theta_\pa}}{\sin{\theta_\pc}}        
        \leq
        \frac{\sin{\theta_\pb}}{\sin{\pth{\theta_\pa + \theta_\pb}}}
        +(1+\eps)
        \frac{\eps}{4}
        \leq%
        \frac{\sin{\theta_\pb}}{(1-\eps/4)\sin{\pth{\theta_\pb}}}
        +(1+\eps)
        \frac{\eps}{4}
        \leq%
        1+\eps.
    ...
    \begin{equation*}
        \dY{\pc}{\pc'} \leq \dY{\pa}{\pc'} \tan \sin \Theta_\pa \leq
        2\beta\dY{\pa}{\pc'} \leq (\eps/8) \dY{\pa}{\pc'}.
    \end{equation*}
  Observe that
    $\cos \theta_\pa \geq 1 - (\theta_\pa)^2/2 \leq 1+\eps^2/4$ as
    $\theta_\pa$ is an angle smaller than (say) $\eps/16$.  We thus
    have that
    \begin{equation*}
        \dY{\pa}{\pc}%
        +%
    ...
    Let
    Consider the case that $\dY{\pa}{\pb}\leq \dY{\pa}{\pc}$.  Let
    $\pb'$ the projection of $\pb$ on $\pa\pc$. We have that
    \begin{equation*}
        \dY{\pb}{\pb'}%
        =%
        \dY{\pa}{\pb} \sin \beta%
    ...
    chosen by being the first point in the cone $c$ encountered by a
    line parallel to the edge opposite of $\pa$. See the red line and
    direction in \figref{tri_cone_edge}. This implies that the angle
    $\theta_\pc$ is at least $\alpha$, and the result then follows.
    
    $\dY{\pa}{\pb}< \dY{\pa}{\pc}$: Denote by $\pc'$ the projection of
    $\pc$ on $\overline{\pa\pb}$. We have that
    ...
    Since $\theta_\pa = O(\eps\alpha)$ we get that for a sufficiently
    large constant the condition
