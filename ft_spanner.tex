...
\begin{theorem}
    ...
    has $O\pth{ n / (\alpha\eps) \bigr.}$ edges.
\end{theorem}
...
\begin{proof}
    ...
    $O\pth{ (\alpha\eps)^{-1}n\log n}$ \cite{c-aaspmp-87}.
\end{proof}
...
\subsubsection{A local spanner for nice polygons}
...
example, a $k$ regular polygon is $t$-nice.

\begin{lemma}
    Given a $t$-nice polygon $\Body$, and a parameter $\epsA$, one can
    cover it by a set $\Traps$ of $O((t/\epsA)^2)$ $\epsA$-narrow
    trapezoids, such that for any two points
    $\pa, \pb \in \partial \Body$ that belong to two edges of $\Body$
    that are not adjacent, there exists a narrow trapezoid
    $\Trap \in \Traps$, such that $\pa, \pb$ are located on the two
    short legs of $\Trap$.
\end{lemma}
\begin{proof}
    We show a somewhat suboptimal but simple construction. A $t$-nice
    polygon has at most $t$ edges.  Let $\ell$ be the sensitivity of
    ...
    $\gamma = \epsA \ell/ \constC$, for some sufficiently large
    constant $\constC$. In addition, place $O(t)$ equally space points
    between any two consecutive points of $\PS$, such that the
    distance between any two consecutive added points is at most (say)
    $\gamma/20t$. Let $\PSA$ be the resulting set from $\PS$ after
    adding all these points.


    Clearly, we have that $|\PS| = O( t /\epsA)$ and
    $|\PSA| = O(t^2/\epsA)$. For a direction $v$, let $\Traps_v$ be
    the decomposition into trapezoids formed by shooting rays from
    inside $\Body$ in the direction of $v$ (or $-v$) from all the
    points of $\PS$. Let $\Traps_v'$ be the set resulting from
    throwing away trapezoids with legs that lie on adjacent edges.  It
    is easy to verify that all the trapezoids of $\Traps_v'$ are
    $\epsA$-narrow.  Let $U$ be the set of all directions induced by
    pairs of points of $\PS \times \PSA$, and let
    $\Traps = \cup_{u \in U} \Traps_u'$. We have that
    ...
    $\pb_1 \pb_2$. Indeed, the angle between two lines supporting
    $\pa_1\pa_2$ and $\pb_1 \pb_2$ is in the range
    $[2\pi/t, \pi - 2\pi/t]$, which readily implies the claim. Thus
    there is a trapezoid in $\Trap$ that contains $\pa$ and $\pb$ on
    their legs, which implies the claim.
\end{proof}
