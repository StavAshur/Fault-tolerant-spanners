...
triangulation contains at most with $\Of( |\PSX| + |\PSY|)$ edges. As
...
\begin{proof}
    ...
    to its right, let $\diskVY{\pa}{\pb}$ be the disk that its
    boundary passes through $\pa$ and $\pb$, and its center has the
    same $x$-coordinate as $\pa$.
    
    In the $j$\th iteration, for $j=2,\ldots, M-1$, consider the set
    of disks
    \begin{equation*}
        D_j = \Set{ \diskVY{\pa_i}{\pb_{j-1}} }{i=1,\ldots, n},
    \end{equation*}
    see \figref{flower}. Let $x_j = n2^{M-j+1} = x(\pb_{j-1})/2$, and
    ...
    y_j)$. Clearly, the point $\pb_j$ lies outside all the disks of
    ...
    potentially also contains the points $\pb_{j-1}, \ldots, \pb_1$,
...
factor of $\Of(1/\epsA)$, by using the algorithm of \lemref{refine:d:w},
...
$(1+\eps)-\LL$ local spanner of size $\Of(\eps^{-3}n\log n)$ in
$\Of(\eps^{-2}n\log n)$ time.(
...
copy of itself.  See \figref{grid}. This grid has $\Of(\gConst^2)$
...
\begin{proof}
    ...
    \smallskip%
    \noindent%
    \itemref{x:bigger} The width of $(1-\delta)\rect$ is at least $x$,
