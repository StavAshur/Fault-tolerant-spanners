...
% \subsubsection{Delaunay triangulation induced by a convex body}

Let $\CC$ be a bounded convex and closed region in the plane (e.g., a
disk).  The \emphi{homothet} of a convex shape $\CC$ is a scaled and
translated copy of $\CC$.  A point set $\PS$ is \emphi{general
   position} for $\CC$, if is four points of $\PS$ lie on the boundary
of a homothet of $\CC$.

\begin{defn}[\cite{cd-vdbcdf-85}]
    \deflab{c:del:triang}%
    %
    Given $\PS$ and $\CC$ as above, the \emphi{$\CC$-Delaunay
       triangulation} of $\PS$ is the graph formed by edges between
    any two points $\pa,\pb\in \PS$ such that there exist a homothet
    of $\CC$ that contains only $\pa$ and $\pb$ and no other point of
    $\PS$.
\end{defn}
...
Observe that for any homothet $C$ (of $\CC$) and two points
$\pa,\pb \in C$, there exists a homothet $C'$ of $\CC$ with $\pa$ and
$\pb$ on its boundary, such that $C'\subseteq C$. Thus, a
straightforward modification of the proof of \clmref{d:t:connected}
...
\section{A local spanner for narrow trapezoids}
\seclab{traps}
...
XXX
\begin{lemma}
    Let $\eps \in (0,1)$ be some parameter, and let $\PX,\PY$ be two
    points sets that are $\eps$-semi separated, and let $\Trap$ be
    $\eps$-narrow trapezoid, with two points $\pa \in \PX$ and
    $\pb \in \PY$ lying on the two legs of $\Trap$. Then, one can
    compute a homothet $\Trap' \subseteq \Trap$ of $\Trap$, such that:
    \begin{compactenumI}
...
\begin{figure}[h]
    \centerline{\includegraphics{figs/narrow_trap}}%
    \caption{}
    \figlab{narrow:trap}
\end{figure}
\begin{proof}
    Assume, that $\diamX{\PX} \leq \eps $
\end{proof}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Proof of \clmref{d:t:connected}}
\apndlab{d:t:c:proof}

\RestatementOf{\clmref{d:t:connected}}%
{%
   \ClaimDelConnected{} }
...
\begin{proof}
    ...
    $\disk'$, see \figref{shrink}. Observe that $\pa$ and $\pc$ are on
