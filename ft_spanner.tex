...
\begin{abstract}
    ...
    regular $k$-gons. In particular, this improves over the known
...
$\G_\PS - \region$ (See \defref{def:residual:graph} for a formal definition of this notation). Surprisingly, as shown by Abam \etal
...
subgraph only if both its endpoints are in $\region$, see \defref{def:residual:graph}).
...
In \apndref{qspd}, we describe a novel pair-decomposition.
...
based shrinking mentioned above). Contrast this with the lower bound (illustrated in \figref{bad:rectangles})
...
\begin{proof}
    ...
    \begin{figure}[h]
        ...
        \caption{An illustration of the settings in the proof of \lemref{w:l:s:regions} with $\Elp$ shown in blue.}
        \figlab{ellipse}
    \end{figure}
...
A graph $\G=(\PS, \EG)$ is a \emphw{$\CC$-local $t$-spanner}
...
\begin{defn}[\cite{cd-vdbcdf-85}]
    ...
    with respect to $\CC$, the \emphi{$\CC$-Delaunay triangulation} of $\PS$,
...
\begin{corollary}[Proof in \apndref{safe:graph:proof}]
	\corlab{safe:graph}
...
\begin{lemma}
    \lemlab{l:s:lower:bound}[Proof in \apndref{l:s:lower:bound:proof}]%
    %
    \LemmaDiskLowerBound
...
\begin{lemma}
    \lemlab{l:b:triangles}[Proof in \apndref{l:b:triangles:proof}]%
    %
...
\begin{lemma}[Proof in \apndref{cone_edge:triangles:proof}]
...
\SaveContent{\LemmaShrinkTriangles}%
{%
...
\begin{lemma}[Proof in \apndref{shrink:triangles:proof}]
...
\begin{lemma}[Proof in \apndref{good:jump:traps:proof}]
...
\begin{lemma}[Proof in \apndref{narrow:traps:decomp:proof}]
...
\begin{proof}
    ...
    $(1+\epsA)$-spanner of \thmref{l:s:triangle} for \PS. Next, we cover $\CC$
    by a set $\Traps$ of $k' = \Of(k^4/\epsA^3)$ $\epsA$-narrow
    trapezoids using \lemref{narrow:traps:decomp}.
...
two edges in the constructed graph for each grid cell (that contains
at least two points), and $\Of( \gConst^2)$ edges overall.
...
\begin{figure}
	...
	\caption{Left: The two rectangles $\rect$, $\rect'$. Right: In
		green $\xSlabX{\rect} \cap \rect'$, the restriction
		of the slab $\xSlabX{\rect}$ to the rectangle
		$\rect'$.}
	\figlab{x:expand}
\end{figure}
...
\begin{proof}
    ...
    \begin{figure}[h]
        ...
        \hfill%
        \phantom{}%
        \caption{An illustration of the proof of \clmref{c:t:connected} in the case that $C$ is a disk.}
        \figlab{shrink}
    \end{figure}

    Otherwise, let $\pc\in \PS$ be a point in the interior of
    $C$. From \lemref{shrink:shrank} we get that there exists
...
\subsection{Proof of \corref{safe:graph}}
\apndlab{safe:graph:proof}

\RestatementOf{\corref{safe:graph}}%
{%
	\CorollarySafeGraph{}
}
...
\subsection{Proof of \lemref{l:b:triangles}}
\apndlab{l:b:triangles:proof}

\RestatementOf{\lemref{l:b:triangles}}%
{%
	\LemmaTriangleLowerBound{}
}
...
\subsection{Proof of \lemref{cone_edge:triangles}}
\apndlab{cone_edge:triangles:proof}

\RestatementOf{\lemref{cone_edge:triangles}}%
{%
\LemmaConeEdgeTriangles{}
}
...
\begin{proof}
	...
	\begin{figure}[h]
		...
		\caption{ The case that $\dY{\pa}{\pb}\leq \dY{\pa}{\pc}$ in \lemref{cone_edge:triangles}. The vector used to determine $\nnZ{i}{\pa}{\cone}$ is shown in red, and denoted $\dir$}
	...
	\begin{figure}[h]
		...
		\caption{The case that $\dY{\pa}{\pb}> \dY{\pa}{\pc}$ in \lemref{cone_edge:triangles}.}%
...
\subsection{Proof of \lemref{shrink:triangles}}
\apndlab{shrink:triangles:proof}

\RestatementOf{\lemref{shrink:triangles}}%
{%
	\LemmaShrinkTriangles{}
}
...
\begin{figure}[h]
	...
	\caption{ An illustration of the shrinking process of \lemref{shrink:triangles}. The three left subfigures are an activation of \lemref{shrink:shrank} for the case that the convex region $\CC$ is a triangle, and the rightmost subfigure is the additional final step.}
	\figlab{shrink:triangle}
\end{figure}


\subsection{Proof of \lemref{good:jump:traps}}
\apndlab{good:jump:traps:proof}

\RestatementOf{\lemref{good:jump:traps}}%
{%
	\LemmaGoodJumpTraps{}
}
...
\begin{figure}[h]
	...
	\caption{Illustration of the settings in the proof of \lemref{good:jump:traps}. Left: A $\epsA$-narrow trapezoid with $\pa$ and $\pb$ on its legs. Center: $\pa$ and $\pb$ are $\epsA$-semi separated
...
\end{figure}
\begin{proof}
	Let $\DT = \DG_\Trap(\PX \cup \PY)$.  \clmref{c:t:connected} implies
...
\subsection{Proof of \lemref{narrow:traps:decomp}}
\apndlab{narrow:traps:decomp:proof}

\RestatementOf{\lemref{narrow:traps:decomp}}%
{%
	\LemmaNarrowTrapsDecomp{}
}
...
\begin{proof}
	...
	$\Body$, and place a minimum set of points $\PS$ on the boundary of
	$\Body$, which includes all the vertices of $\Body$, and such that the distance between any consecutive pair of points is in
	the range $[\constA, 2\constA]$, where
	...
	\begin{figure}[h]
		...
		\hfill%
		\phantom{}%
		\caption{The points of $\PS$ (round), and all the
			points added to $\PS$ in order to create $\PSA$ (square). On the right, a ``vertical''
	...
	\begin{figure}[h]
		...
		\caption{The height of the triangle $\triangle \pa_1 \pa_2 \pb_2$ is
			minimized as $\pb_2$ and $\pa_1$ are moved to vertices of $\Body$.}%
		\figlab{points:move}
	\end{figure}
