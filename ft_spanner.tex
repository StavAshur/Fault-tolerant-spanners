...
%	that are contained in $F$
	...
	a geometric local $t$-spanner of $\PS$ with respect to $\FF$, is a
	subgraph of $G(\PS)$, the complete geometric graph over $P$, that is
	also a $t$-spanner of every subgraph of $\PS$ that is induced by a
	region $F\in \FF$, where the subgraph induced by $F$ is the subgraph
	that contains only the points and the edges of $G\PS)$ that are
	`contained in $F$.
...
A \emphw{fault-tolerant spanner} for $\FF$ and $\PS\subseteq \Re^d$, is a graph $\G=(\PS, \EG)$, such that
...
In the same spirit, a graph $\G=(\PS, \EG)$ is a \emphw{local spanner} for $\FF$ and $\PS$
...
%As one of the most relevant works, Abam et al. [1] presented
%a region-fault tolerant (1 + ")-spanner G of size O(n log n) for a point set P
%w.r.t. convex regions, i.e. for any convex region R the graph G􀀀R is a (1+")-
%spanner for K(P) 􀀀 R. Here, for points p and q the edge fp; qg may not exist
%in K(P)􀀀R, and indeed the length of the shortest path from p to q in G􀀀R
%is guaranteed to be at most (1 + ") times the length of the shortest path
%from p to q in K(P)􀀀R . When convex regions are half planes, K(P)􀀀R is a
%complete graph on the remaining points. Therefore, this problem is the inverse
%of the local spanner problem, and we already have (1+")-local spanner of size
%O(n log n) for the familly of halfplanes.
...
In \secref{squares} we show a construction similar to that of \secref{disks}, but prove that for the case of local square spanners we are able to produce a spanner of size $\Of\pth{\eps^{-3} n \log n}$, that is, independent of the spread of the point set. 
...
convex shape $\CC$. This family of regions is also known as homothets of $\CC$. Formally, the homothet of $\CC$ of center $o$ and scaling $\lambda$ is the set $\{\lambda x+o : \pa \in \CC\}$. See \figref{homothet}. While the Delaunay triangulation is not well
...
\begin{proof}
	Due to \thmref{c:t:construct_delaunay_graph}, \clmref{c:t:linear_size}, and \clmref{c:t:connected}, we can use the algorithm in subsection \ref{subsec:disk_construction} verbatim, switching only the conventional Delaunay triangulation with the $\CC$-Delaunay graph. Since the $\CC$-Delaunay graph has linear complexity for every convex shape $\CC$ the size of the spanner can be bounded using the same asymptotical bounds as the disk local spanner, and since the $\CC$-Delaunay graph has linear size, the bounds on the number of edges hold as well. The correctness proof of \thmref{main:1} uses only the connectivity of the Delaunay triangulation, and thus the same proof suffices.
...
\begin{figure}[h]
	...
	\caption{For any fixed $t>1$ we can take a "thin" right angle triangle $\triangle$, and create a point set as shown in the figure, such that every point in the set on the lower left side of the figure must be connected by an edge to each of the points in the upper part of the figure (which are of exponentially increasing $x$-coordinate) in order to create a $\triangle$-local $t$-spanner.}
	\figlab{tri_low_bd}
\end{figure}
...
\begin{claim}
	For any fixed $\eps \in (0,1)$, there exists a set $\PS$ of $n$ points and a triangle $\triangle$, such that the $\triangle$-local $(1+\eps)$-spanner of $\PS$ requires $O(n^2)$ edges. 
\end{claim}

\begin{proof}
	We first take $\triangle$ to be the right angle triangle $\left((0,1), (0,2^n+1), (1,n) \right)$. Next, we describe two sets of points $A$ and $B$ of size $n$ each (with some abuse of notation), such that every point in $A$ must be connected to $O(n)$ points of $B$ in any $\triangle$-local $(1+\eps)$-spanner. 
	Let $A = \{a_i:=(-i, i/n) ~|~ 0\leq i \leq n \}$, and let $B = \{b_i:=(n-i, 1+2^i) ~|~ 1\leq i \leq n \}$. Notice that for every point $a\in A$ and index $i\in \{0,..,n\}$, There exist a homothet $\triangle'$ of $\triangle$ such that $B\cap \triangle'=\{b_i,...,b_n\}$. See \figref{tri_low_bd} for some intuition.
	
	We now show that $|\overline{ab_{i+1}}| + |\overline{b_{i}b_{i+1}}| \geq (1+\eps)|\overline{ab_{i}}|$ for $O(n)$ indices. First we make a couple of simple observations:
	
	\begin{enumerate}
		\item $1+\eps \leq 2$,
		\item $|\overline{ab_{i+1}}|\geq 2^{i+1}$,
		\item $|\overline{b_{i}b_{i+1}}| \geq 2^i$,
		\item and $|\overline{ab_{i}}| \leq $.
	\end{enumerate}
...
The input is a point set $\PS$ of $n$ points in the plane, a triangle $triangle$, and an
approximation parameter $\eps \in (0,1)$. Let $c_1,c_2$ and $c_3$ be the 3 cones with an apex at the origin that coincide each with one of the angles of $\triangle$ when translating its respective vertex to the origin. We also denote the orthogonal direction to the edge opposite of $\alpha_i$ by $v_i$. See \figref{tri_cones} for an illustration. Let $C_1,C_2$, and $C_3$ be partitions of each of these cones into cones of angle at most $O(\eps)\cdot \alpha_i$ (for the appropriate $i$).
...
\begin{lemma}
	...
	\begin{enumerate}
		\item {
			...
			|\overline{\pa\pc}|%
			+%
			(1+\eps)|\overline{\pb\pc}|%
			\leq%
			(1+\eps)|\overline{\pa\pb}|%
			$
			,and
		}
		\item {
...
\begin{proof}
	...
	We now consider two possible cases, either $|\overline{\pa\pb}|\leq |\overline{\pa\pc}|$ or $|\overline{\pa\pb}|> |\overline{\pa\pc}|$. Notice that  since the angle of the cone $c$, and therefore $\theta_\pa$ as well, is smaller   than the smallest angle of $\triangle$, we have that $|\overline{\pb\pc}|\leq    \max \{|\overline{\pa\pc}|,|\overline{\pa\pb}|\}$.
	
	$|\overline{\pa\pb}|\leq |\overline{\pa\pc}|$:
	Denote by $\pb'$ the projection of $\pb$ on $\overline{\pa\pc}$. We have that 
	
	$$
	|\overline{\pb\pb'}|%
	=%
	\sin\theta_\pa |\overline{\pa\pb}|%
	=%
	\sin\theta_\pc |\overline{\pb\pc}|%
	\implies%
	|\overline{\pb\pc}|%
	=
	\frac{\sin\theta_\pa}{\sin\theta_\pc}|\overline{\pa\pb}|%
	$$
	...
	|\overline{\pb'\pc}|%
	=%
	\cos\theta_\pa |\overline{\pa\pb}|%
	+%
	\cos\theta_\pc |\overline{\pb\pc}|%
	=%
	\cos\theta_\pa |\overline{\pa\pb}|%
	+%
	\frac{\sin\theta_\pa}{\sin\theta_\pc}|\overline{\pa\pb}|%
	=%
	(\cos\theta_\pa + \frac{\sin\theta_\pa}{\sin\theta_\pc})|\overline{\pa\pb}|
	$$
	...
	$$
	\iff%
	|\overline{\pa\pb}|(\cos\theta_\pa + \frac{\sin\theta_\pa}{\sin\theta_\pc})%
	+%
	(1+\eps)\frac{\sin\theta_\pa}{\sin\theta_\pc}|\overline{\pa\pb}|%
	\leq%
	(1+\eps)|\overline{\pa\pb}|%
	$$
	...
	The last piece of this part is the fact that point $\pc$ was chosen by being the first point in the cone $c$ encountered by a line parallel to the edge opposite of $\pa$. See the red line and direction in \figref{tri_cone_edge}. This implies that the angle $\theta_\pc$ is at least $\alpha$, where $\alpha = \min \{\alpha_1,\alpha_2\alpha_3\}$, the result then follows.

	$|\overline{\pa\pb}|< |\overline{\pa\pc}|$:
	...
	|\overline{\pb\pc'}|%
	=%
	\sin\theta_\pa|\overline{\pa\pc}|%
	+%
	(|\overline{\pa\pb}|-|\overline{\pb\pc'}|)%
	$$
	...
	\sin\theta_\pa|\overline{\pa\pc}|%
	+%
	(|\overline{\pa\pb}|-\cos\theta_\pa|\overline{\pa\pc}|)%
	$$
	...
	\overline{\pa\pc}|
	+%
	\frac{1}{\cos\theta_\pa-\sin\theta_\pa}|\overline{\pc\pb}|%
	\leq%
	\frac{1}{\cos\theta_\pa-\sin\theta_\pa}|\overline{\pa\pb}|
	$$
